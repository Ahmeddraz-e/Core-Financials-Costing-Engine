...
export default function Sidebar({ activeTab, setActiveTab, lang, onLogout, stats, userRole, userPermissions, username }: SidebarProps) {
  ...
  return (
    <aside id="sidebar_main" className="w-72 bg-[#ebf4ff] dark:bg-[#070e1d] text-slate-700 dark:text-slate-300 flex flex-col h-screen select-none border-[#d1e4fc] dark:border-slate-900 border-r shrink-0 z-30" dir={isAr ? "rtl" : "ltr"}>
      <div className="p-6 border-b border-[#d1e4fc] dark:border-slate-900 flex flex-col items-center justify-center bg-[#f4f8fe]/60 dark:bg-[#0a1526]/50">
        <h2 className="text-3xl font-black tracking-tight text-[#0056b3] dark:text-[#00c6ff] font-sans">LODing ERP</h2>
        <span className="text-[9px] text-[#081a3e]/60 dark:text-slate-400 font-extrabold block tracking-wider uppercase mt-1">{isAr ? "نظام إدارة وتخطيط الموارد" : "SYSTEMS & SOFTWARE SOLUTIONS"}</span>
      </div>
        ...
        {groups.map((group, gi) => {
          ...
          return (
            <div key={gi} className="space-y-1.5">
              <span className="px-3 text-[10px] font-bold text-[#0056b3]/70 dark:text-slate-500 uppercase tracking-widest block">{isAr ? group.titleAr : group.titleEn}</span>
              <nav className="space-y-0.5">
                {visible.map(item => {
                  const active = activeTab === item.id;
                  const Icon = item.icon;
                  return (
                    <button key={item.id} id={"sidebar_item_" + item.id} onClick={() => setActiveTab(item.id)} className={"w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-150 " + (active ? "bg-blue-600 text-white shadow-sm font-bold" : "text-slate-600 dark:text-slate-400 hover:text-blue-700 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-900/50")}>
      ...
      <div className="p-4 border-t border-[#d1e4fc] dark:border-slate-900 space-y-2 bg-[#f4f8fe]/60 dark:bg-transparent">
        <div className="flex items-center gap-3 p-2 rounded-xl bg-white/80 dark:bg-slate-900/40 border border-[#d1e4fc] dark:border-slate-800/40">
          <div className="h-9 w-9 rounded-full bg-[#ebf4ff] dark:bg-slate-800 flex items-center justify-center font-bold text-blue-600 dark:text-sky-400 border border-[#b8d6fc] dark:border-slate-700 text-xs">{initials}</div>
          <div className="flex-1 min-w-0">
            <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">{username || (isAr ? "مدير النظام" : "System Administrator")}</h4>
            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold block truncate">{roleLabel}</span>
          </div>
        </div>
        <button id="logout_btn" onClick={onLogout} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/20 transition-all duration-150">
