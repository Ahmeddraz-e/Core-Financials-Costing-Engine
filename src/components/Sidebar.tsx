...
export default function Sidebar({ activeTab, setActiveTab, lang, onLogout, stats, userRole, userPermissions, username }: SidebarProps) {
  ...
  return (
    <aside id="sidebar_main" className="w-72 bg-[#0f172a] dark:bg-[#070e1d] text-slate-200 dark:text-slate-300 flex flex-col h-screen select-none border-[#1e293b] dark:border-slate-900 border-r shrink-0 z-30" dir={isAr ? "rtl" : "ltr"}>
      <div className="p-6 border-b border-[#1e293b] dark:border-slate-900 flex flex-col items-center justify-center bg-[#1e293b]/30 dark:bg-[#0a1526]/50">
        ...
        <span className="text-[9px] text-slate-400 dark:text-slate-400 font-extrabold block tracking-wider uppercase mt-1">{isAr ? "نظام إدارة وتخطيط الموارد" : "SYSTEMS & SOFTWARE SOLUTIONS"}</span>
      </div>
