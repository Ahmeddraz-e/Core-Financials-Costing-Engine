...
export function seedDatabase(db: Database.Database): void {
  ...
  if (accountCount.cnt === 0) {
    ...
    const insertTreasury = db.prepare(`INSERT OR IGNORE INTO treasuries (id, nameAr, nameEn, balance) VALUES (?, ?, ?, ?)`);
    insertTreasury.run('cb-1', 'خزينة الفرع الرئيسي - الكاشير', 'Main Branch Cashier', 0);

    const insertBank = db.prepare(`INSERT OR IGNORE INTO bank_accounts (id, accountNumber, bankNameAr, bankNameEn, balance) VALUES (?, ?, ?, ?, ?)`);
    insertBank.run('ba-1', 'EG1234567890123456789', 'البنك الأهلي المصري - حساب جاري', 'NBE - Current Account', 0);
